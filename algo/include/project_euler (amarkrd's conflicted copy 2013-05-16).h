...
namespace euler
{
	...
	{
		bool * arr = new bool[1500000];
		std::memset(arr,1,1500000*sizeof(bool));
		arr[2] = true;
		arr[3] = true;
	...
	void program_50()
	{
		...
		for(unsigned int i = 0; i < primesVec.size(); i++)
		{
			for(unsigned int j = primesVec.size()-1; j > i; j--)
			{
				...
				if(it != primesSet.end() && max < ((int)j-(int)i))
				{
					max = j-i;
					maxnum = (*it);
	...
	bool islargeprime(long long x,std::set<int>& primes)
	{
		...
		{
			std::set<int>::iterator it = primes.find(x);
			if(it == primes.end())
				return false;
		}
		return true;
	...
	void program_63()
	{
		...
		for(int i = 2 ; i < 10; i++)
		{
			count = count + (1/(1-std::log10(1.0*i)));
		}
		std::cout<<count<<std::endl;
		/*for(; i < 50; i++)
		{
	...
	void program_70()
	{
		...
		for(unsigned int i = 0; i < primes.size() && primes[i] < 10000000; i++)
		{
			for(unsigned int j = i+1; j < primes.size() && primes[j] < 10000000 && (((long)primes[i]*(long)primes[j]) < 10000000); j++)
			{
				...
				if(totient < 0)
					break;
				if(are_anagram(totient, (long)primes[i]*(long)primes[j]) && ((long)primes[i]*(long)primes[j]*1.0)/totient < min)
				{
